...
SceneManager::SceneManager(ConfigManager* inConfigManager) {
    ...
    for( auto& mySceneFileLocation : myConfigManager->myScenes ) {
        size_t scene_hash_key = std::hash<std::string>{}(std::string(mySceneFileLocation));
        std::cout << "Indexing scene " << scene_hash_key << std::endl;
        myScenes[scene_hash_key] = mySceneLoader->doLoadScene(myFilepathManager->mySceneDir + mySceneFileLocation);
    }
}
